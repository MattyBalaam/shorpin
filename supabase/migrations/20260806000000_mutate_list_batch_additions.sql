-- Reconnect sync (app/routes/list/list.tsx's performRebaseSync) collapses a
-- whole offline session into a single mutate_list call. Until now, only a
-- single new-item value (`new`) could be inserted per call, so if a user
-- added more than one item while offline, only the most recent addition
-- survived — the others sat in the submitted `items[]` array under
-- client-generated ids that this function has always treated as
-- unrecognized (a no-op, not an insert).
--
-- Adds `newItems`: a jsonb array of values, each inserted as its own row.
-- `new` (single value) is unchanged and still used for the common
-- online, one-at-a-time add.
DROP FUNCTION IF EXISTS public.mutate_list(text, jsonb, bigint);
...
BEGIN
  ...
  -- must run before the add-inserts below: newly-added rows obviously
  -- aren't in the submitted array either, and would otherwise look deleted
  -- in the very same request that created them.
  ...
  FOR v_new_item IN
    SELECT value
    FROM jsonb_array_elements_text(COALESCE(p_payload -> 'newItems', '[]'::jsonb))
  LOOP
    IF COALESCE(v_new_item, '') = '' THEN
      CONTINUE;
    END IF;
    ...
    INSERT INTO public.list_items (id, list_id, value, state, updated_at, sort_order)
    VALUES (gen_random_uuid(), v_list_id, v_new_item, 'active', p_mutated_at, v_next_sort_order);
