-- Fixes add-item detection in mutate_list, broken when the client migrated
-- to Conform 1.20's formal intent handlers (see app/routes/list/intents.ts).
-- That migration removed the "new-submit" payload field in favor of the
-- dedicated p_intent parameter, but this function was never updated to
-- match — new items were silently never inserted.
--
-- delete-item/undelete-item detection is untouched: the action
-- (app/routes/list/list.server.ts) translates Conform's dispatcher-serialized
-- delete-item("<uuid>") back into the legacy "delete-item-<uuid>" string
-- before calling this RPC, so it stays unaware of Conform's wire format.
...
BEGIN
  ...
  IF p_intent = 'add-item' AND COALESCE(v_add_value, '') <> '' THEN
