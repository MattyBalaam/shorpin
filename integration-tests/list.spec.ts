...
test("edited icon does not appear on unedited items while another item is being deleted", async ({
	...
	const { intercepted, release } = await holdLoader(
		page,
		/\/lists\/shopping\.data/,
	);
	...
	await expect(
		page.locator("li").filter({ has: page.getByLabel("Edit Eggs") }),
	).not.toContainText(" edited");
});
...
test("new item is marked in the list since last opening", async ({
	browser,
	ctx,
}) => {
	const ownerContext = await browser.newContext({ baseURL });
	...
	try {
		...
		await expect(
			ownerPage
				...
				.filter({ has: ownerPage.getByLabel("Edit Butter") }),
		).toHaveCount(1);
		await expect(
			ownerPage
				.locator('[data-new="true"]')
				.filter({ has: ownerPage.getByLabel("Edit Milk") }),
		).toHaveCount(0);
	} finally {
		await ownerContext.close();
		await collabContext.close();
...
test("reordering items does not create unread or new markers for self", async ({
	page,
	ctx,
}) => {
	await login(page, ctx.ownerEmail);
	...
	await expect(
		page
			.locator('[data-new="true"]')
			.filter({ has: page.getByLabel("Edit Milk") }),
	).toHaveCount(0);
});
