...
const listNewPath = (slug: string) => `/list-new/${slug}`;

/**
 * Navigate straight to the new list route and wait for client hydration.
 ...
 */
async function openListNew(page: Page, slug: string) {
  await page.goto(listNewPath(slug));
  await page.waitForURL(listNewPath(slug));
  await expect(page.locator(`html[data-hydrated-path="${listNewPath(slug)}"]`)).toBeAttached();
}

test("list-new shows all items", async ({ page, ctx }) => {
  await login(page, ctx.ownerEmail);
  await openListNew(page, "shopping");
...
test("list-new empty list shows the placeholder prompt", async ({ page, ctx }) => {
  await login(page, ctx.ownerEmail);
  await openListNew(page, "owner-empty");
...
test("owner can add an item on list-new", async ({ page, ctx }) => {
  await login(page, ctx.ownerEmail);
  await openListNew(page, "owner-empty");
...
test("edited icon does not appear on unedited items while another item is being deleted", async ({
  ...
  const { intercepted, release } = await holdLoader(page, /\/list-new\/shopping\.data/);
...
test("item added by collaborator appears as unread for owner on home page", async ({
  ...
  try {
    ...
    await openListNew(ownerPage, "shopping");
    ...
    await openListNew(collabPage, "shopping");
...
test("reordering items does not create unread or new markers for self", async ({ page, ctx }) => {
  ...
  await page.route("**/list-new/shopping", async (route) => {
  ...
  await page.unroute("**/list-new/shopping");
