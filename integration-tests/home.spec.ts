...
test("owner can reorder lists from home", async ({ page, ctx }) => {
	...
	for (let attempt = 0; attempt < 3; attempt++) {
		...
		if (!fromBox || !toBox) {
			throw new Error(
				"Unable to determine drag handle positions for home reorder",
			);
		}
...
test("collaborator sees their lists plus the shared list", async ({
	page,
	ctx,
}) => {
...
test("shows an error message when list creation fails", async ({
	page,
	ctx,
}) => {
	await login(page, ctx.ownerEmail);
...
test("collaborator sees unread badge for shared list not yet opened", async ({
	page,
	ctx,
}) => {
	await login(page, ctx.collabEmail);
