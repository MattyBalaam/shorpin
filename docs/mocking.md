...
`pnpm test:integration` runs [`scripts/test-e2e.mjs`](../scripts/test-e2e.mjs), which finds free ports starting from `9001` (mock) and `5174` (app) before starting Playwright. This prevents port conflicts when `pnpm dev` or another test run is already using those ports.
