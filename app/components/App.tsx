...
import { Items, type ItemProps } from "./items";
import { InfoModal } from "./info-modal";
import getList from "../../src/utils/getList";
import setList from "../../src/utils/setList";
...
type Error = "loading error" | "saving error";
...
class App extends Component<{}, State> {
	...
	handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
		...
		this.doAState(({ items, newItem }) => ({
			items: [...items, { id: crypto.randomUUID(), name: newItem }],
			newItem: "",
			undos: [],
		}));
	};
	...
	render() {
		const { info, infoType, items, newItem, itemsAdded, itemsRemoved, undos } =
			this.state;
