...
import { Link as RouterLink, useNavigation, useResolvedPath } from "react-router";
...
export function Link({ variant = "link", className, children, ...props }: LinkProps) {
  const { state, location } = useNavigation();
  const resolved = useResolvedPath(props.to);

  const isLoading = state === "loading" && location.pathname === resolved.pathname;
