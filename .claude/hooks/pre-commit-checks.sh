...
if [ -n "$NON_TRIVIAL" ]; then
  ...
  if [ -n "$RUN_ALL" ]; then
    echo "Running full integration suite (shared infra or layout changed)..." >&2
    if ! pnpm test:integration >&2; then
      deny "Integration tests failed — see output above. Fix before committing."
    fi
  else
    # Accumulate matching specs
    SPECS=""
    echo "$STAGED" | grep -qE '^app/routes/home'      && SPECS="$SPECS integration-tests/home.spec.ts"
    echo "$STAGED" | grep -qE '^app/routes/list'     && SPECS="$SPECS integration-tests/list.spec.ts integration-tests/delete.spec.ts integration-tests/config.spec.ts"
    echo "$STAGED" | grep -qE '^app/routes/delete'    && SPECS="$SPECS integration-tests/delete.spec.ts"
    echo "$STAGED" | grep -qE '^app/routes/auth/'     && SPECS="$SPECS integration-tests/auth.spec.ts"
    echo "$STAGED" | grep -qE '^app/routes/sign-ups'  && SPECS="$SPECS integration-tests/sign-ups.spec.ts"
    # Add any changed spec files themselves (exclude supabase-* which needs its own runner)
    CHANGED_SPECS=$(echo "$STAGED" | grep -E '^integration-tests/.*\.spec\.ts$' | grep -v 'supabase-')
    [ -n "$CHANGED_SPECS" ] && SPECS="$SPECS $CHANGED_SPECS"

    if [ -z "$SPECS" ]; then
      # Non-trivial changes but no route match — run everything to be safe
      echo "Running full integration suite (unrecognised file pattern)..." >&2
      if ! pnpm test:integration >&2; then
        deny "Integration tests failed — see output above. Fix before committing."
      fi
    else
      ...
      SPECS=$(echo "$SPECS" | tr ' ' '\n' | sort -u | grep -v '^$' | tr '\n' ' ')
      echo "Running integration: $SPECS" >&2
      # shellcheck disable=SC2086
      if ! pnpm test:integration -- $SPECS >&2; then
        deny "Integration tests failed — see output above. Fix before committing."
      fi
    fi
  fi
...
printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","permissionDecisionReason":"Typecheck, lint, formatting, and integration tests passed."}}'
exit 0
